...
def train_gnn_classifier(model, train_dataset, val_dataset, device, epochs=200, lr=1e-5):
    ...
    for epoch in range(epochs):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            batch.y = batch.y.squeeze().long()  # 去除一个维度
            ...
            train_probs.extend(torch.softmax(out, dim=1).cpu().detach().numpy())  # 保存所有类别的概率
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_probs.extend(torch.softmax(out, dim=1).cpu().detach().numpy())  # 保存所有类别的概率
...
def main():
    ...
    # 示例数据集（需要替换为实际数据集）
    # # TODO enzymes: Data(x=[90, 3], edge_index=[2, 254], y=[1] y=0-5, node_label=[90])
    data_name = "enzymes"
    train_dataset = ENZYMES('train')
    valid_dataset = ENZYMES('valid')
    test_dataset = ENZYMES('test')
